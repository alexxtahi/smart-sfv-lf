                        ...
                        <div class="row">    
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="number" min="1" class="form-control" id="quantite_denombree" placeholder="Qté dénombrée">
                                </div>
                            </div>
                            <div class="col-md-2">
                ...
                <div class="modal-body ">
                   ...
                   <div class="row">
                       ...
                       <div class="col-md-2">
                           <div class="form-group">
                               <label>En Stock</label>
                               <input type="number" class="form-control" name="quantite_en_stocke" ng-model="article.quantite_en_stocke" id="en_stock_add" placeholder="Qté en stock" readonly>
                           </div>
                       </div>
                       <div class="col-md-2">
                           <div class="form-group">
                               <label>Qt&eacute; d&eacute;nombr&eacute;e *</label>
                               <input type="number" min="0" name="quantite_denombree" ng-model="article.quantite_denombree" class="form-control" placeholder="Qté dénombrée" required>
                           </div>
                       </div>
                   </div>
                </div>
...
<script type="text/javascript">
    ...
    $(function () {
            ...
            if(depot!=""){
                ...
                $.getJSON("../boutique/liste-article-by-unite-in-depot/" + depot , function (reponse) {
                        ...
                        $.each(reponse.rows, function (index, depot_article) { 
                            $('#article').append('<option data-libellearticle= "' + depot_article.description_article + '"  value=' + depot_article.id_article + '>' + depot_article.description_article + '</option>')
                        });
                });
            }else{
                $("#code_barre").val("");
            ...
            $.getJSON("../parametre/liste-articles-grouper-code_barre-in-depot/" + code_barre + "/" + depot , function (reponse) {
                $.each(reponse.rows, function (index, retour) { 
                    ...
                    $.getJSON("../boutique/liste-unites-by-depot-article/" + depot + "/" + retour.article.id , function (reponse) {
                        ...
                        $.each(reponse.rows, function (index, colis) { 
                            $('#unite_add').append('<option data-libelleunite= "' + colis.unite.libelle_unite + '" value=' + colis.unite.id + '>' + colis.unite.libelle_unite + '</option>')
                        });
                    })
                });
            })
        ...
        $('#unite').change(function(){
            ...
            $.getJSON("../boutique/find-article-in-depot-by-unite/" + article_id + "/" + depot_id + "/" +  unite_id, function (reponse) {
                $.each(reponse.rows, function (index, article) { 
                    ...
                    $("#date_peremption").val(article.date_peremptions);
                });
            })
        });
        ...
        $(".add-row").click(function () {
            if($("#article").val() != '' && $("#quantite_denombree").val() != '' && $("#unite").val() != '') {
                var code_barre = $("#code_barre").val();
                ...
                var date_peremption = $("#date_peremption").val();
                    ...
                    var DataArticle = {'id':idTablle, 'articles':articleId, 'unites':uniteId,'quantite_en_stocks':quantite_en_stock,'quantite_denombrees':quantite_denombree,'date_peremptions':date_peremption};
        ...
        $("#formAjout").submit(function (e) {
            ...
            var $ajaxLoader = $("#formAjout .loader-overlay");
             ...
             if (ajout==true) {
                ...
                var methode = 'POST';
                var url = "{{route('boutique.update-inventaire')}}";
                var formData = new FormData($(this)[0]);
             }
        ...
        $("#formAjoutArticle").submit(function (e) {
            ...
            if (ajoutArticle==true) {
                var methode = 'POST';
                var url = "{{route('boutique.detail-inventaires.store')}}";
             }else{
    ...
    function updateRow(idInventaire){
        ...
        $tableArticle.bootstrapTable('refreshOptions', {url: "../boutique/liste-details-inventaire/" + idInventaire});
